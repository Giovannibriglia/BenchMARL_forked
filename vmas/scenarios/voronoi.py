    ...
    def reward(self, agent: Agent) -> Tensor:
        ...
        if self.shared_rew:
            if is_first:
                for j in range(self.world.batch_dim):
                    ...
                    for i, a in enumerate(self.world.agents):
                        reward[j, i] = self.voronoi.computeCoverageFunctionSingleEnv(vor, self.pdf[j],
            ...
            for j in range(self.world.batch_dim):
                robots_j = robots[:, j, :]
                vor = self.voronoi.partitioning_single_env(robots_j)
                self.single_rew = self.voronoi.computeCoverageFunctionSingleEnv(vor, self.pdf[j],
                                                                                self.world.agents.index(agent), j)
...
class VoronoiPolicy(BaseHeuristicPolicy):
    def __init__(self, env, *args, **kwargs):
        ...
        self.Kp = 0.8
        self.voronoi = VoronoiCoverage(self.grid_spacing, self.cells_range, self.xdim, self.ydim, self.device,
                                       self.centralized)
...
class VoronoiCoverage:
    ...
    def mirror(self, points, xmin, xmax, ymin, ymax):
        square_corners = torch.tensor([(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)], device=self.device,
                                      dtype=torch.float32)
    ...
    def computeCoverageFunction(self, agent_id):
        ...
        for i in range(self.worlds_num):
            ...
            reward[i] = torch.sum(weights * torch.linalg.norm(self.xy_grid[bool_val] - self.agents[agent_id, i],
                                                              axis=1) ** 2) * self.grid_spacing ** 2
    ...
    def computeCoverageFunctionSingleEnv(self, voro, pdf_global, agent_id, env_id):
        ...
        reward = torch.sum(
            weights * torch.linalg.norm(self.xy_grid_tot[bool_val] - robot, axis=1) ** 2) * self.grid_spacing ** 2
        return -reward
    ...
    def computeCentroidSingleEnv(self, vor, pdf):
        ...
        if self.centralized:
            ...
            x_grid = torch.linspace(robot[0] - self.cells_range * self.grid_spacing,
                                    robot[0] + self.cells_range * self.grid_spacing, self.nxcells)
            y_grid = torch.linspace(robot[1] - self.cells_range * self.grid_spacing,
                                    robot[1] + self.cells_range * self.grid_spacing, self.nycells)
            xg, yg = torch.meshgrid(x_grid, y_grid)
